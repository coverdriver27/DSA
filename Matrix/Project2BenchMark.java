...
public class Project2BenchMark 
{
interface FPFunction
  ...
  public static double trapezium2(double a, double b, int n, FPFunction f)
  {
   double range = checkParamsGetRange(a, b, n); 
   double nFloat = (double)n;
double sum = 0.0;

sum = IntStream.range(1,n).parallel().mapToDouble(i -> i * 1.0).
...
return sum * range / nFloat;
  ...
  private static void testFunction(String fname, double a, double b, int n, FPFunction f)
  {
   ...
   trapezium(a, b, n, f);
    ...
    System.out.println("For the size n:" + n + " the time it takes to run is: " + algTime);
    return;
  }
  ...
  public static void main(String[] args)
  {
   for(int x=0; x<15; x++)
   {
     testFunction("x^3", 0.0, 1.0, 1000000000*(x+1), new FPFunction() {
